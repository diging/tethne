...
"""
    This module contains various methods(views of a webpage)
    with respect to User Login, creating datasets
...
def login():
    ...
    try:
        ...
        pass
    #print "comes out", request.form , "uname", username ,\
    ...
    if 1:
          ...
          dbroot = conn.open().root()
          try: 
              print "####values()", dbroot['userdb'].values(), \
                    "#####keys",dbroot['userdb'].keys(), type(dbroot['userdb'])
              if name in dbroot['userdb'].keys():
                  for key in dbroot['userdb'].keys(): #Traverse through keys
                      ...
                      if key != 'admin' and name != 'admin':
                                 if name == key:
                                  # Need to get username from the \
                                        #formpassword = sha256(password)
                                  dbpswd = dbroot['userdb'][key].password
                                  loginpswd = sha256(userpassword).hexdigest()
                                  if dbpswd == loginpswd:                                 
...
def create_datasets():
    ...
    if request.method == 'POST'  :
        ...
        print "Object", slice
    # Commenting because it was discussed not to use tethne Command Line
#        
