...
class wos_library:
    def __init__(self, identifier, inputFile):
        ...
        self.size = None                #   Number of records in the library. Set by build()
...
#   Reads self.data file, and builds self.library, a list of wos_objects.
    def build(self):
        ...
        #   We're still in the middle of a record...
                ...
                if (prefix is not 'XX') and (prefix is not ''):
                    if prefix is '  ':          #   If there is no prefix, the line is part of the field to which the previous line belonged.
                        prefix = last_prefix
                    else:                       #   If there is a prefix, the line is part of the next field.
                        cache[prefix] = []
                #   XX means that the line was probably blank, above.
                    if prefix is not 'XX':
...
#   Generates an author-paper network, stored in author_graph
    def authors(self):
        self.authors = []           #   List of authors (by AU)
        self.papers = []            #   List of papers (by wosid)
        self.author_graph = []      #   List of tuples (author_index, paper_index), from self.authors and self.papers
        self.author_papers = {}     #   Dictionary of lists of papers by author_index (from self.authors)
        
        for entry in self.library:
            self.papers.append(entry.wosid)
            p_index = self.papers.index(entry.wosid)
            self.library[entry.wosid] = entry
...
#   Generates a co-author network, stored in self.coauthor_graph
    def coauthors(self, graph_type):
        self.coauthor_graph = []    #   List of tuples (author_index, author_index), from self.authors
...
#   was internalNetwork()
    def internal_citations(self, parameters = None):
        i = 0                       #   Count 'em
        self.internal = []          #   Start clean 

        if len(self.relations) > 0:     #   Must run build() and citationNetwork()
...
#   Was compareAbsolute()
    def bibliographic_coupling(self, threshold, path, start_year, end_year): #   path is output directory
        histogram = {}
        ...
        self.couplings = (couplings, nodes)
...
#   Returns xgmml dynamic networks of various types
    ...
    #   Bibliographic coupling
        if network_type is 'coupling':
            couplings, nodes = self.couplings
